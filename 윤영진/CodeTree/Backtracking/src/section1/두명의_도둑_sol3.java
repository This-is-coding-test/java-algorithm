...
public class 두명의_도둑_sol3 {

    static int N, M, C;
    static int[][] map;

    // 전처리를 진행할 배열 -> 이미 계산이 된 경우에는 값, 아닌 경우에는 -1
                ...
                for (int sx2 = 0; sx2 < N; sx2++) {
                    for (int sy2 = 0; sy2 < N; sy2++) {
                        if (isPossible(sx1, sy1, sx2, sy2)) {
                            // 선택된 도둑마다 가장 많은 무게 합
                            result = Math.max(result, findMax(sx1, sy1) + findMax(sx2, sy2));
                        }
                    }
                }
            }
    ...
    private static int findMax(int sx1, int sy1) {
        if (bestValue[sx1][sy1] != -1) return bestValue[sx1][sy1];
